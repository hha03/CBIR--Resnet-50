from flask import Flask, request, jsonify, send_from_directory, render_template, flash, redirect, url_for,  json
...
@app.route('/get-list-categories', methods=['GET'])
def get_list_categories():
    ...
    return jsonify([{"id": category[0], "name": category[1]} for category in categories])
...
def add_image():
    ...
    if file:
        ...
        # Lưu danh mục vào bảng `food_categories`
        if categories:
            # Giả sử categories là một chuỗi JSON, bạn có thể chuyển đổi thành danh sách
            categories_list = json.loads(categories)  # Nếu categories là một chuỗi JSON
            categoryIds = get_category_ids(categories_list)
            for categoryId in categoryIds:
                query_insert_category = """
                INSERT INTO food_category (food_id, category_id) 
                VALUES (%s, %s);
                """
                cursor.execute(query_insert_category, (food_id, categoryId))
            conn.commit()
